...
@section('title', $variant === 'receipt' ? 'Transfer Confirmed' : 'Transfer Invoice')

@section('preheader', $variant === 'receipt'
    ? 'Your airport transfer is confirmed. Your receipt is attached.'
    : 'Your airport transfer invoice is attached.')
...
@section('content')
    ...
    @if ($recipient === 'driver')
        ...
        <p style="color: #6b7280; font-size: 14px; margin-bottom: 16px;">
            Hi {{ $driverName ?? 'Driver' }}, you have been assigned to an airport transfer. Please find the booking details and {{ $variant }} attached.
        </p>
    @else
        <h1 style="font-size: 22px; font-weight: 700; color: #111827; margin-bottom: 6px;">
            {{ $variant === 'receipt' ? 'Transfer Confirmed!' : 'Transfer Invoice' }}
        </h1>
        <p style="color: #6b7280; font-size: 14px; margin-bottom: 16px;">
            Hi {{ $customerName }},
            @if ($variant === 'receipt')
                your airport transfer is confirmed. Please find your receipt attached.
            @else
                thank you for booking an airport transfer. Your invoice is attached - payment is due before or at the time of transfer.
            @endif
        </p>
    @endif

    {{-- PDF notice --}}
    <div style="background-color:#f5f3ff;border:1px solid #c4b5fd;border-radius:8px;padding:10px 16px;margin-bottom:24px;">
        <span style="font-size:13px;color:#7c3aed;font-weight:600;">&#128206; Your transfer {{ $variant }} is attached to this email as a PDF.</span>
    </div>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse; margin-bottom: 28px;">
        ...
        <tr>
            ...
            <td style="font-size: 13px; color: #111827; font-weight: 600; padding: 4px 0;">{{ $flightNumber }}@if ($airline) &mdash; {{ $airline }}@endif</td>
        </tr>
        @endif
        @if ($vehicleName && $recipient !== 'driver')
        <tr>
