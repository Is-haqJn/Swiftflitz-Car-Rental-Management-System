...
class SendReturnReminderJob implements ShouldQueue
{
    ...
    /**
     * Send a return reminder email and in-app notification to the customer,
     * and optionally a copy to the manager if their email setting is enabled.
     */
    public function handle(
        ...
        if (! empty($staffIds)) {
            $notificationService->send(
                $staffIds,
                'return_reminder',
                'Vehicle Return Due Soon',
                "Rental {$rental->reference} for {$rental->customer->name} is due for return on {$rental->return_date->format('M d, Y')}.",
                ['rental_id' => $rental->id, 'reference' => $rental->reference, 'action_url' => "/management/rentals/{$rental->id}"],
            );

            foreach (User::findMany($staffIds) as $recipient) {
                if ($recipient->email && $notificationService->shouldSendEmail($recipient, 'return_reminder')) {
                    Mail::to($recipient->email)->queue(new ReturnReminderMail($rental));
                }
            }
        }

        if ($rental->customer->email) {
            Mail::to($rental->customer->email)->queue(new ReturnReminderMail($rental));
        }

        $whatsAppService->notifyReturnReminder($rental);
        $smsService->notifyReturnReminder($rental);
        $whatsAppService->notifyAdminReturnReminder($rental);
        $smsService->notifyAdminReturnReminder($rental);
    }
