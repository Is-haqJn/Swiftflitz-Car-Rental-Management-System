...
function overdueGrantPermission(User $user, string $permission): void
{
...
it('notifies users with rentals.view_all permission in the rental branch', function () {
    ...
    overdueGrantPermission($branchUser, 'rentals.view_all');
    ...
    $job = new SendOverdueAlertJob($rental);
    $job->handle(
    ...
    expect(AppNotification::where('user_id', $branchUser->id)->where('type', 'overdue_alert')->count())->toBe(1);
});
...
it('always notifies the assigned rental manager_id regardless of permissions', function () {
    ...
    /* Manager is in a different branch and has no rental permissions */
    ...
    expect(AppNotification::where('user_id', $assignedManager->id)->where('type', 'overdue_alert')->count())->toBe(1);
});
...
it('does not notify users from other branches', function () {
    ...
    overdueGrantPermission($branchAUser, 'rentals.view_all');
    ...
    expect(AppNotification::where('user_id', $branchAUser->id)->where('type', 'overdue_alert')->count())->toBe(0);
});
...
it('notifies global admin with rentals.view_all and no branch assignment', function () {
    ...
    overdueGrantPermission($globalAdmin, 'rentals.view_all');
    ...
    expect(AppNotification::where('user_id', $globalAdmin->id)->where('type', 'overdue_alert')->count())->toBe(1);
});
