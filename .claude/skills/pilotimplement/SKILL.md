---
name: pilotimplement
description: Direct implementation with Copilot. Use this skill whenever the user types /pilotimplement or asks to "pilotimplement" a task. Triggers on any message containing /pilotimplement followed by a task description. The skill skips plan mode, follows the same session workflow and memory checks as pilotplan, researches the codebase as needed, executes focused implementation steps with subagents using the best available model, and validates the result before updating memory.
license: MIT
metadata:
  author: swiftflitz-team
---

# PilotImplement Skill

One-phase workflow: **Copilot researches, then implements immediately.**

## When This Skill Is Active

User typed `/pilotimplement <task>`. Run the full workflow below from start to finish.
...
2. Smallest safe file set — cross-reference NODES list
3. Focused step-by-step execution order
4. Risks/gotchas — especially god nodes
5. Test-first: for each step, (a) test file + `it()` cases (Red first), then (b) implementation
   files (Green). Red-Green-Refactor per step.
...
Implement immediately after research. Break into focused steps per cohesive file set.
...
**For complex steps, spawn a subagent (omit `model`):**
...
4. Update `.claude/CLAUDE.md` — durable architectural decisions (parent agent only, not subagents).
...
- **Same session workflow and test rigor as pilotplan.**
